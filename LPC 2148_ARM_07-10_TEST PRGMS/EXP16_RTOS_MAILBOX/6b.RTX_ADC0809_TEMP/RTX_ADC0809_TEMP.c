...
char lcd_msg1[] = "TEMP SNR LM335";
char lcd_msg2[] = "TEMP(C)=";
float ana_output=0.0, adc_out=0.0, temp_out=0.0, avg_out = 0.0;
...
__task void send_task(void) {
	...
	while(1){     
		...
		tmp1 = 0x80;
		lcd_com();
		delay();						// display message lcd_msg2[] = "TEMP SNR LM335" ;
		lcd_puts(lcd_msg1);
		delay();
		
		tmp1 = 0xC0;
		lcd_com();					   	// display message lcd_msg2[] = "TEMP(C)=" ;
		delay();
		lcd_puts(lcd_msg2);
		delay();
		...
		sprintf(mptr->ana_op ,"%.2f" ,temp_out);
