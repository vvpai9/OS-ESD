...
unsigned char msg1[]="RELAY\r", msg2[]="LED's\r";
...
__task void task1(void) {
	...
	while (1) {
    	...
    	if(ret == OS_R_OK) { //A token was available and the function returned immediately
			...
			LPC_GPIO0->FIOSET = 0x02000000;	//RLY ON
			os_dly_wait(100);
			LPC_GPIO0->FIOCLR = 0x02000000;	//RLY OFF
			os_dly_wait(100);
		}
 	...
 	OS_RESULT ret;
  	while (1) {
		...
		if(ret == OS_R_SEM) {	// The calling task has waited until a semaphore became available.
			...
			LPC_GPIO0->FIOCLR = 0x00000FF0;					// CLEAR P0.4 to P0.11 to turn ON LEDs 
			os_dly_wait(50);
			LPC_GPIO0->FIOSET = 0x00000FF0;					// SET  P0.4 to P0.11 to turn OFF LEDs 
			os_dly_wait(50);
		}	
    	// Return a token back to a semaphore. 
	   	os_sem_send (semaphore1);
...
int main(void) {
	...
	LPC_GPIO0->FIODIR |= 0x02000FF0;		// P0.4 to P0.11 and P0.25 as output
	LPC_GPIO0->FIOSET = 0x00000FF0;			// Initially turn OFF all LED
