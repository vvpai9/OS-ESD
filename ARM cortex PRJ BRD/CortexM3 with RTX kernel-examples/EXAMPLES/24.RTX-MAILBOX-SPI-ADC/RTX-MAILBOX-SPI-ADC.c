 ...
 *      Purpose: SPI ADC using Mailbox concept
 *----------------------------------------------------------------------------*/
...
unsigned int avg,temp=0;
unsigned char spi_flag=0, Dummy=0;
char buf[20];
	...
	T_MEAS *mptr;
 	...
 	while(1){
		...
		{							
			...
			os_evt_wait_or( EVT_KEY , 0xffff);	  	// wait for an interrupt event

        	LPC_SPI->SPDR = 0x80;					// Select channel1 in single ended mode & MSB first format
...
int main (void) {                     	/* program execution starts here       */
	...
	LPC_GPIO0->FIODIR |= SSEL;								// CHIPSELECT LINE MADE AS O/P P0.16
